...
-Encoded Answer String Validation (for absolutes: type-1 & 2) --> (DONE!)
 -→ Generate EAS function
 -→ Test EAS function
 -→ get rid of .wa/.ra references in code
 -→ enforce if encoded string loaded before run-time (no .wa/.ra in HTML, already encoded!)
 ---→ FIND CODE: #raware!
 
V0.9000

-Encoded Answer String Validation (for type-3) (DONE)
 -→ Encrypt/decrypt strings answers
 ---→ FIND CODE: #raware2!

v0.9001

-Encoded Answer String Validation (for type-4)
 -→ Encrypt/decrypt strings answers
 ---→ FIND CODE: #raware3!

*/
cvcheck="PMigXnXMhQ4";//ncat -> "jI-kpVh6e1U"; //pbjt -> jScuYd3_xdQ  //h5-0 -> hwhvByj8YG8  //valkyries -> P73Z6291Pt8  //MrT -> Mr2ptPJrLNw  //trololo -> sCNrK-n68CM(TJL4Y3aGPuA)  //bcat -> -ZOiX6cIT8o  //winerr -> MJbTjBLEKBU  //saxman -> Kas6akz1jWU  // roxbury -> 09m0B8RRiEE  //letitfrozen -> 1IlhWwTiPfY  //hckeynight -> YsU_Q6xwcAw  //mxico -> q-Rqdgna3Yw // fix IE trololo -> sCNrK-n68CM //Banana Boat -> PMigXnXMhQ4
/*
...
function qsBeta(objAcc,paramObj) {
	...
	//
	//DEBUG MODE
	debugMode = (paramObj.debugMode!=undefined)?paramObj.debugMode:false;
	
	//STOPS INITS (IF FALSE) UNTIL EXPRESSLY INSTRUCTED (useful for manually changing tracking data before launching)
	autoStart = (paramObj.autoStart!=undefined)?paramObj.autoStart:true;
	...
	//CHEAT MODE
	cheatMode = (paramObj.cheatMode!=undefined)?paramObj.cheatMode:false;
	
	//ENCODE ANSWERS AUTOMATICALLY
	encodeAnswers = (paramObj.encodeAnswers!=undefined)?paramObj.encodeAnswers:false;
	
	/*WIP*/
	if(cheatMode){
		if(encodeAnswers){fCl('Answers are encoded, cheatMode not will not work!');}
		//type-0
		//type-1,type-2
	...
	//illegalCharsList = "░▒▓╠╣┌┐║♥♦♠♣";
	var illegalCharsList = (paramObj.illegalCharsList!=undefined)?paramObj.illegalCharsList:"¼½¾Ð¶¢®©ª¤£¦§±þ§Þß¥º·¹²³¨÷|";
	// ║ -> ¦
	// ┌ -> §
	// ┐ -> ¤
	// ╠ -> ±
	...
	var animBaseDuration = (paramObj.animBaseDuration!=undefined)?paramObj.animBaseDuration:200;
	
	//SECURITY "SEED" VALIDATION INTEGER
	var masterIntTester = (paramObj.masterIntTester!=undefined||paramObj.masterIntTester<4)?paramObj.masterIntTester:33; //KEEP IT OVER 3 AND UNDER 100 PLS! THE HIGHER THE NUMBER THE MORE COMPLEX TO DECODE (FOR END USER SNOOPING AROUND THAT IS!)
		...
		//if(eval(this.qsComputeProgression(ancestryIDARR[0],"qat")+1) == eval(this.qsComputeProgression(ancestryIDARR[0],"qtot")) && qOffset === 1){ //#7 XYZ
			...
			$("#"+this.activities[ancestryIDARR[0]].id).find('.qs-submit-activity').click(function(){
				if(!$(this).hasClass('qs-disabled')){
					...
					//eval(objAcc).qsAnswerQuestion($("#"+$(this).attr('data-passthrough')),'auto');//#7 XYZ
	...
	this.qsAnswerQuestion = function (elem,qMode){
		...
		switch(question_type){
			...
			case "type-2":
				...
				$(this.activities[ancestryIDARR[0]].exercises[ancestryIDARR[1]].questions[ancestryIDARR[2]].answers.elem).find('input[type="checkbox"]').each(function (){
					...
					raCheck =(encodeAnswers)?(eval(objAcc).qsTestAgainstEncodedValidationString('gonogo',$(this).attr('data-encoded-rep'))!=$(this).prop('checked')):($(this).hasClass('ra') != $(this).prop('checked'));
					//if($(this).hasClass('ra') != $(this).prop('checked')){ //#raware!
					//if(eval(objAcc).qsTestAgainstEncodedValidationString('gonogo',$(this).attr('data-encoded-rep'))!=$(this).prop('checked')){
					if(raCheck){
						right_answer = false;
						return false;
					}
			...
			case "type-3":
				...
				right_answer = true;
					...
					var testAgainst = (encodeAnswers)?qsObj.qsTestAgainstEncodedValidationString('encrypt',$(this).attr('data-encoded-rep')):$(this).attr('data-rep'); 
					...
					$(this).addClass((right_answer)?'qs_good_answer':'qs_wrong_answer');
		...
		if(!secondChanceNeeded){
			...
			if(questionFeedbackType == "deferred" || questionFeedbackType == "none"){
				...
				//alert(A+" - "+B+" - "+C+" - "+D);
				//TRUE - TRUE - 1 - FALSE = HAE THE SUBMIT FINAL APPEAR INSTEAD (qs-final-submit-placeholder)
						...
						if(this.activities[ancestryIDARR[0]].force_insert_final_submit){
							...
							$('#'+this.activities[ancestryIDARR[0]].id).find('.qs-submit-activity').click(function(){
								if(!$(this).hasClass('qs-disabled')){
									...
									eval(objAcc).qsNextQuestion($('#'+$(this).attr('data-pass-id')));
									
									//eval(objAcc).qsNextQuestion("#"+this.activities[current_activity].exercises[current_exercise_ID].questions[current_question_ID].id,1)
								}
							});
						}
					}else{
	...
	this.qsTestAgainstEncodedValidationString = function (mode,testString){
		...
		switch(mode){
			case "gonogo":
				pass = false;
				if(Number(testString)%masterIntTester===0){
					pass=true;
				}
				break;
			case "encrypt":
	...
	function fQsEvDp(evt,pars){
		fCl('fQsEvDp('+evt+')');
		try{
			fQsEventDispatcher(evt);
		}
		catch(err){
			fCe('Did not find function...');
		}
	}
...
$(document).ready(function() {
	...
	qsObjParams = {
		"debugMode":false, //true|false: Displays logs
		...
		"qsdata_override":"qsdata_" //*WIP TO THIS WILL BE ADDED A SUFFIX FR||EN AND .json (lowercase!)
