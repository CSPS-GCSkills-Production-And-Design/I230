...
/*$document.on( "focusin", "body", function( event ) {
	...
	if ( extendedGlobal && $.magnificPopup.instance.currItem &&
		...
		// Close the popup
		//$.magnificPopup.close(); CSPS fix lightbox close
	}
} );*/
...
$document.on( "mouseleave", selector + " .menu", function( event ) {
	...
	globalTimeout = setTimeout( function() {
		/*menuClose( $( event.currentTarget ).find( ".active" ), true ); //CSPS SJ removing close on timeout */
	}, hoverDelay );
} );
	...
	if ( !( event.ctrlKey || event.altKey || event.metaKey ) ) {
		...
		// Menu item is within a menu bar
			...
			// Enter sub-menu
				...
				$subMenu.children( "li" ).find( menuItemSelector.replace("> a", "> a[aria-hidden!='true']").replace("summary", "summary[aria-hidden!='true']") ).eq(0).trigger( focusEvent );
			...
			// Letters only
				...
				selectByLetter(
					which,
					$menuItem.parent().find( "> ul > li > a[aria-hidden!='true']" ).get() //CSPS SJ added [aria-hidden!='true']
				);
			}
		...
		// Menu item is not within a menu bar
			...
			if ( which === 38 || which === 40 ) {
				event.preventDefault();
				menuIncrement(
					$menu.children( "li" ).find( menuitemSelector.replace("> a", "> a[aria-hidden!='true']").replace("summary", "summary[aria-hidden!='true']")), //CSPS SJ PRIORITY add something like [aria-hidden!='true']
					$menuItem,
					which === 38 ? -1 : 1
				);
			...
			// Escape, left / right arrow without a submenu
				...
				if ( $parentMenu.attr( "role" ) === "menubar" ) {
					...
					// Left / right key = Next / previous menu bar item
						...
						menuIncrement(
							$parentMenu.find( "> li > a[aria-hidden!='true'][disable!='true']"), //CSPS SJ added [aria-hidden!='true']
							$menuLink,
							which === 37 ? -1 : 1
						);
					}
